...
# Author: Sambal0x
#
# Instructions : ./subdomain-quickie.sh <domain.com>
...
touch subd-takeover.txt
echo "[+] We have $(cat $1-subdomains.txt| wc -l) subdomains.. Now analysing results with Subjack..."
for i in $1-subdomains.txt; do /root/go/bin/subjack -w $i -c /root/go/src/github.com/haccer/subjack/fingerprints.json -o subd-takeover.txt; done 

# Send results to slack's web hook
# echo "[+] Sending results to slack ..."
# var="$(cat subd-takeover.txt)"
# curl -X POST -H 'Content-type: application/json' --data "{'text':'## Subdomain Takeover Results ##\n$var'}" \
